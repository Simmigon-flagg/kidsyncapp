...
export const useAuthStore = create((set) => ({
    ...
    register: async (username, email, password) => {
        ...
        set({ isLoading: true })
        try {
            const response = await fetch("http://192.168.1.238:3000/api/v1/auth/register", {
                method: "POST",
                headers: {
            ...
            if (!data.user || !data.token) throw new Error("Missing user or token in response")
    ...
    handleGoogleAuth: async (accessToken) => {
        ...
        try {
            ...
            const response = await fetch("http://192.168.1.238:3000/api/v1/auth/google", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
    ...
    login: async (email, password) => {
        set({ isLoading: true })
        try {
            const response = await fetch("http://192.168.1.238:3000/api/v1/auth/login", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
